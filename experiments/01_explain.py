...
def _get_cache_filename(args, CACHE_DIR) -> str:
    if args.module_name == "fmri":
        ...
        if args.checkpoint_module == "decapoda-research/llama-30b-hf":
            suffix = '_opt'
        return join(CACHE_DIR, "cache_ngrams", f"{args.module_name}_{args.subject}{suffix}.pkl")
...
def add_main_args(parser):
    ...
    parser.add_argument(
        "--module_num",
        type=int,  # good task is d3_13_water or d3_16_hillary
        default=0,
        help="number of module to select",
    )
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    assert (
        not args.module_num == args.module_num_restrict
    ...
    if args.module_name == "fmri":
        ...
        mod = sasc.modules.old_fmri_module.OldFMRIModule(voxel_num_best=args.module_num)
        r["fmri_test_corr"] = mod.corr
    elif args.module_name == "dict_learn_factor":
    ...
    if args.subsample_frac < 1:  # and not args.module_name == 'fmri':
        assert (
            False
        ), "dont subsample data right now, since explain_ngrams is using caching"
    ...
    llm = imodelsx.llm.get_llm(args.checkpoint, CACHE_DIR=join(CACHE_DIR, args.checkpoint))
    (
        explanation_strs,
